...
const ParticipantsPanel: React.FC<ParticipantsPanelProps> = ({
   activeParticipants = [],
    ...
    useEffect(() => {
        ...
        const currentUserIds = new Set(streams.keys());
        ...
        return (
            <div
                ...
                className={`relative bg-black rounded-lg overflow-hidden transition-all duration-300 ${
                    isUserSpeaking ? 'ring-4 ring-green-400 ring-opacity-80' : ''
                }`}
            ...
            >
                                ...
                                ref={(el) => {
                                    if (el) {
                                        ...
                                        if (streams.has(participant.id)) {
                                            el.srcObject = streams.get(participant.id)!;
                ...
                {/* Name overlay */}
                <div className={`absolute bottom-2 left-2 px-2 py-1 rounded-md backdrop-blur-sm ${
                    isUserSpeaking ? 'bg-green-600 bg-opacity-90' : 'bg-black bg-opacity-60'
                }`}>
                    <span className="text-white text-sm font-medium flex items-center gap-2">
                        {isUserSpeaking && (
                            <span className="flex gap-0.5">
                                <span className="w-1 h-3 bg-green-300 rounded animate-pulse"></span>
                ...
                <div className="absolute top-2 right-2 flex gap-1">
                    <div className={`p-1.5 rounded-full backdrop-blur-sm ${
                        hasVideo ? 'bg-green-500' : 'bg-gray-500'
                    }`}>
                    ...
                    <div className={`p-1.5 rounded-full backdrop-blur-sm ${
                        isCurrentUser && !isMuted ? 'bg-green-500' : 'bg-red-500'
                    }`}>
                        <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 24 24" fill="white">
