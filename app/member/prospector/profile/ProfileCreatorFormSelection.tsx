...
const sections: Section[] = [
    ...
    { id: 'format', label: 'Format pracy' },
    { id: 'preferences', label: 'Preferencje' }
];

const ProfileCreatorFormSelection: React.FC<ProfileCreatorFormSelectionProps> = ({
    activeSection,
    setActiveSection
        ...
        <div className="flex space-x-1 mb-8 bg-app-white border border-app-dark-white p-1 rounded-md">
            {sections.map(section => (
                <button key={section.id} type="button" onClick={() => setActiveSection(section.id)}
                    className={`flex-1 py-2 px-4 rounded-md text-base font-medium transition-colors ${activeSection === section.id ? 
...
export default ProfileCreatorFormSelection;